...
State values include `not_firing`, `firing`, `pending_resolve`, `errored`, `snoozed`, and `broken`. The `broken` state indicates the alert was auto-disabled after too many consecutive evaluation failures. You can reset a broken alert via the API to clear the failure counter and schedule an immediate recheck.
