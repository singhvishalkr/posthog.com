...
The `system.logs_alerts` table contains [log alert](/docs/logs) configurations that monitor log volume and notify when thresholds are breached. Alerts use an N-of-M evaluation model where the alert fires when `datapoints_to_alarm` (N) out of the last `evaluation_periods` (M) checks breach the threshold.
...
The `system.logs_views` table contains saved [log views](/docs/logs) — named filter configurations that users create to quickly access frequently-used log queries.
