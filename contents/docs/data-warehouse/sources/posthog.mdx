...
State values include `not_firing`, `firing`, `pending_resolve`, `errored`, `snoozed`, and `broken`. The `broken` state indicates the alert was auto-disabled after too many consecutive evaluation failures. You can reset a broken alert directly from the alert modal or the list view dropdown menu in PostHog. Resetting clears the failure counter and schedules an immediate recheck.
