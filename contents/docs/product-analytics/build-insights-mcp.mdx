...
- `Run all insights on my Growth Metrics dashboard and summarize the results.`
...
| `dashboard-insights-run` | Run all insights on a dashboard and return their results. Supports an LLM-friendly text table format with `format=optimized`, or raw JSON with `format=json`. Use `refresh` to control caching (`force_cache`, `blocking`, `force_blocking`). |
