...
Trends insights support up to **3 breakdowns** at a time. Other insight types support a single breakdown (funnels support a single cohort breakdown). When you hit this limit, the **+ Add breakdown** button is disabled.

If you need more flexibility, combine properties into a single breakdown using SQL expressions, or convert your insight to SQL for unlimited breakdowns.

### Combine properties with SQL

To break down by browser and operating system together, use a SQL expression like:
...
### Convert to SQL for unlimited breakdowns

Click **Edit in SQL editor** to convert your insight to a [SQL insight](/docs/product-analytics/sql). SQL insights have no breakdown limits.

> **Note:** Funnels support only **1 breakdown** (and only one cohort breakdown). Retention and paths are also limited to one breakdown. When you need to analyze more dimensions, use SQL expressions or SQL insights.
