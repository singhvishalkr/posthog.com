...
When prompt fetches fail, you can report them automatically to PostHog Error Tracking. This surfaces API issues like 404s (typos in prompt names), 403s (permission problems), auth errors, and network timeouts in your Error Tracking dashboard instead of logging them.

To enable error tracking, pass `capture_errors=True` when initializing `Prompts`:
...
# Enable error tracking for prompt fetch failures
prompts = Prompts(posthog, capture_errors=True)

# Fetch failures now appear in PostHog Error Tracking
template = prompts.get('my-prompt', fallback='default prompt')
```

When `capture_errors=True`:

- Failed fetches are reported via `capture_exception()` before falling back to cache or the fallback value
- Each captured error includes metadata – prompt name, version, and PostHog host
- The SDK handles errors gracefully – if error reporting itself fails, it doesn't affect your fallback logic

Error tracking requires a PostHog client. If you initialize `Prompts` directly without a client, enabling `capture_errors` has no effect.
