...
When using the JavaScript SDK with `withMetadata: true`, you can pass the metadata directly from the prompt result:
...
For Python or when not using `withMetadata`, pass the values directly:
