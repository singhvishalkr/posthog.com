...
<CalloutBox icon="IconShield" title="Automatic PII scrubbing" type="fyi">

PostHog can automatically detect and redact common PII patterns (emails, Bearer tokens, credit card numbers, Stripe keys) at ingest time. This adds a safety net for accidental logging of sensitive data. See [PII scrubbing](/docs/logs/pii-scrubbing) to enable it.
