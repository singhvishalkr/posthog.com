...
By default, `reset()` preserves the device ID so [device-bucketed feature flags](/docs/feature-flags/device-bucketing) stay stable after logout. To also reset the device ID (for example, on shared devices), pass `true` as the second parameter:
...
The SDK generates and persists a stable device ID for [device bucketing](/docs/feature-flags/device-bucketing). To retrieve it:

```javascript
const deviceId = posthog.getDeviceId()
```

This is useful when your backend evaluates device-bucketed flags and needs the same device ID as the mobile app.
