...
By default, `reset()` preserves the device ID so that [device-bucketed feature flags](/docs/feature-flags/device-bucketing) stay stable across logout and login. If you want to also reset the device ID (for example, on shared devices or kiosks), pass `true`:
...
To retrieve the current device ID, call `getDeviceId()`. This is useful if your backend also evaluates feature flags with [device bucketing](/docs/feature-flags/device-bucketing).
