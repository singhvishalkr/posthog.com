...
The `logs` config controls how browser logs are captured and sent to PostHog via OpenTelemetry. See the [Logs usage documentation](/docs/libraries/js/usage#logs) for details on using `posthog.logger` and `posthog.captureLog()`.

| Attribute | Description |
|-----------|-------------|
| `serviceName`<br/><br/>**Type:** String<br/>**Default:** `'unknown_service'` | The service name used in [OpenTelemetry resource attributes](https://opentelemetry.io/docs/specs/otel/resource/semantic_conventions/) for logs. Customize this to distinguish logs from different frontend applications. |
| `environment`<br/><br/>**Type:** String<br/>**Default:** `undefined` | The deployment environment (e.g., `'production'`, `'staging'`). Attached as a resource attribute to all logs. |
| `serviceVersion`<br/><br/>**Type:** String<br/>**Default:** `undefined` | The version of your application. Attached as a resource attribute to all logs. |
| `resourceAttributes`<br/><br/>**Type:** Object<br/>**Default:** `undefined` | Additional [OpenTelemetry resource attributes](https://opentelemetry.io/docs/specs/otel/resource/semantic_conventions/) to attach to all logs (e.g., `'host.name'`, `'cloud.region'`). |
