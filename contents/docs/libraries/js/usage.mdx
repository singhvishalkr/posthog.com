...
Use the `captureLog` method to send structured log entries from your browser to PostHog. Logs are sent to PostHog's [Logs](/docs/logs) product as OTLP-compliant JSON.
...
| `attributes` | object | Structured attributes to attach to the log entry |
| `resource_attributes` | object | Resource-level attributes (e.g., service name, version) |
...
If you're using distributed tracing, you can correlate browser logs with backend traces:
