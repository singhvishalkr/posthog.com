...
Send structured log entries from your browser to PostHog's [Logs](/docs/logs) product. Logs are delivered as OTLP-compliant JSON and can be correlated with session replays, user activity, and distributed traces.

### Configuration

Configure log settings during SDK initialization:

```js-web
posthog.init('<ph_project_api_key>', {
    logs: {
        serviceName: 'checkout-frontend',     // defaults to 'unknown_service'
...
The simplest way to send logs is with the `posthog.logger` methods:

```js-web
posthog.logger.info('Checkout completed', { order_id: 'ord_789' })
posthog.logger.error('Payment failed', { error_code: 'E001' })
posthog.logger.warn('Inventory running low', { sku: 'WIDGET-42' })
posthog.logger.debug('Cache hit', { key: 'user_preferences' })
```
...
For more control, use `captureLog` directly:
...
posthog.captureLog({
    ...
    trace_id: 'abc123def456',  // optional, for OTel correlation
    span_id: 'span789'         // optional
})
```
...
| `level` | string | Severity level: `trace`, `debug`, `info`, `warn`, `error`, `fatal` |
| `attributes` | object | Structured attributes to attach to the log entry. Supports strings, numbers, booleans, arrays, and objects. |
| `trace_id` | string | Optional trace ID for correlating logs with distributed traces |
| `span_id` | string | Optional span ID for correlating logs with specific spans |

### Automatic context

PostHog automatically attaches context to each log entry:

- `distinctId` – The current user's distinct ID
- `sessionId` – The current session ID (for linking to [Session Replay](/docs/session-replay))
- Current URL
- Active [feature flags](/docs/feature-flags)

### Buffering and rate limiting

Logs are buffered and flushed every three seconds or when the buffer reaches 100 records. On page unload, remaining logs are flushed using `sendBeacon` to ensure delivery.

To prevent runaway logging from impacting your bill, the SDK enforces a rate limit of 1,000 logs per flush interval. When exceeded, additional logs are dropped and a warning is logged to the console.

### Example: Correlating with distributed traces

If you're using OpenTelemetry or another distributed tracing system, pass trace context to correlate browser logs with backend spans:
