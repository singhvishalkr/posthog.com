...
The `.survey-box` element also includes a `data-question-index` attribute that indicates which question is currently displayed (zero-indexed). You can use this to apply different styles to different questions or track survey progress in custom implementations:

```css
/* Style the first question differently */
.survey-box[data-question-index="0"] {
  background: #f5f5f5;
}
```
