...
Time series data often has gaps where no events occurred. The `WITH FILL` clause fills these gaps automatically, making it easier to query logs, traces, metrics, and other time-based data.

Add `WITH FILL` to an `ORDER BY` expression to specify the range and step for gap-filling:
...
GROUP BY hour
ORDER BY hour WITH FILL FROM toStartOfHour(now() - interval 7 day) TO toStartOfHour(now()) STEP interval 1 hour
```

### WITH FILL syntax

The full syntax for `WITH FILL` is:

```sql
ORDER BY column WITH FILL FROM start TO end STEP increment
```

- **FROM** - The starting value for filling (optional)
- **TO** - The ending value for filling (optional)
- **STEP** - The increment between filled values (optional)

If you omit these parameters, defaults are used based on the column's data type.

### INTERPOLATE for non-sorted columns

Use `INTERPOLATE` at the end of the `ORDER BY` clause to specify how non-sorted columns are filled:
...
INTERPOLATE (event_count AS 0, unique_users AS 0)
```

Without `INTERPOLATE`, non-sorted columns return default values (typically 0 or empty) for filled rows. With `INTERPOLATE`, you can:

- Set a specific value: `INTERPOLATE (column AS 0)`
- Repeat the previous row's value: `INTERPOLATE (column)`
