...
1. After adding your email address, PostHog shows DNS records for **SPF**, **DKIM**, and **MAIL FROM**.
...
<CalloutBox icon="IconInfo" title="DMARC is recommended but optional" type="fyi">

PostHog also shows a **DMARC** record in your email settings. Adding DMARC can improve deliverability by helping receiving mail servers trust your domain, but it's not required for verification. Your email connector will work without it.
...
| Spam folder issues? | Ensure SPF/DKIM are valid. Consider adding DMARC for better deliverability (optional). |
