...
<<<<<<< HEAD
Every workflow starts with a trigger. Triggers determine when your workflow runs. There are three types of triggers:
||||||| 4d77cfa7f
...
|--------------|-------------|
| Event trigger | Runs when a captured [PostHog event](/docs/data/events) occurs (e.g. `signed up`) |
| Webhook trigger | Programmatically start a workflow via an HTTP request |
| Schedule trigger | Runs on a recurring schedule (e.g. every Monday at 9am) |
||||||| 4d77cfa7f
| Trigger type | Description |
|--------------|-------------|
...
Schedule triggers run your workflow on a recurring schedule without targeting specific persons. They're ideal for cron-style automations like generating weekly reports, sending Monday morning team summaries, or running periodic data syncs.

> **Important:** Schedule triggers run without a person or event context. Actions that reference person properties like `{{person.properties.email}}` won't have access to user data. If your workflow needs to target specific users, use a batch trigger instead.

To set up a schedule trigger:

1. Create a new workflow and select **Schedule** as the trigger type.
2. Set your schedule frequency (hourly, daily, weekly, or monthly).
3. Choose a start date and timezone for when the schedule should begin.
4. Enable the workflow to start running on schedule.

Schedule triggers have three statuses displayed next to the trigger dropdown:

| Status | Description |
|--------|-------------|
| Active | The schedule is running and will fire at the next scheduled time |
| Paused | The schedule is temporarily stopped (the workflow is disabled) |
| Completed | The schedule has finished its run (e.g. a one-time schedule that already fired) |

If a completed schedule's configuration is updated, it automatically resets to active and begins running again.

> **Note:** Schedule triggers create a synthetic `$workflow_scheduled` event when they fire, which you can use for tracking workflow executions in your analytics.
