...
The React Native SDK handles device bucketing automatically. On first SDK init, it generates a device ID, persists it to storage, and includes it in flag evaluation requests. The device ID persists across `identify()` and `reset()` calls.
