...
Device bucketing is currently in beta. JavaScript web and React Native support it automatically. Other SDKs support it when you send the device ID as `$device_id` in person properties when evaluating flags, including with local evaluation.
...
The React Native SDK handles device bucketing automatically. On first SDK init, it generates a device ID, persists it to storage, and includes it in flag evaluation requests. The device ID stays stable across `identify()` and `reset()` calls.
...
If your backend also evaluates feature flags, send `deviceId` with the request so the server uses the same bucketing value as the mobile app.

To reset the device ID (for example, on shared devices or kiosks), pass `true` as the second parameter to `reset()`:

```react-native
// Keep the device ID (default behavior)
posthog.reset()

// Reset the device ID too
...
In JavaScript web and React Native:

- `posthog.reset()` keeps `$device_id`. Device-bucketed flags stay stable even after logout.
- To create a new `$device_id`, use `posthog.reset(true)` in the browser SDK or `posthog.reset(undefined, true)` in the React Native SDK. Use this on shared devices or kiosks when each session should be treated as a new device.
