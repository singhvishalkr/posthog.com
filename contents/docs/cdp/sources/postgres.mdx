...
4. If you need to connect through an SSH tunnel, enable and configure it (optional):
    ...
    - **Require TLS through tunnel?** - Requires TLS encryption for the database connection through the tunnel. Enabled by default. Disable this if your Postgres server doesn't support SSL but is only accessible through the encrypted SSH tunnel.
5. Click **Link**
...
If you're connecting through an SSH tunnel and your Postgres server doesn't support SSL, disable **Require TLS through tunnel?**. The SSH tunnel itself encrypts the connection.
