...
CDC uses your database's native replication to capture changes in real-time. It's the recommended sync method for databases that support it (like [Postgres](/docs/cdp/sources/postgres)) because it:

- Captures inserts, updates, and deletes in real-time
- Avoids full table scans
- Has minimal impact on database performance

CDC requires specific database configuration. See the source-specific documentation for prerequisites and setup instructions.
