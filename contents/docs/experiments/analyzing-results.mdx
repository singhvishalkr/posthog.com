...
- **Chance to win** (Bayesian) or **p-value** (Frequentist): The probability that this variant outperforms the control—not a percentage lift. For example, "92% chance to win" means there's a 92% probability this variant is better, not that it's 92% higher.
