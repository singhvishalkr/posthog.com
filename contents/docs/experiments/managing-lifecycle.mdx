...
- A new feature flag that is disabled by default

If a feature flag with the same key already exists in the target project and has matching variants, that existing flag is reused instead of creating a new one.
