...
You can add property filters to event triggers to narrow which events start a recording. For example, trigger recordings only when a specific button is clicked or when an event has certain property values. This works the same as [property filters](/docs/data/property-filters) elsewhere in PostHog.
